...
| `CLOUDFLARE_ACCOUNT_ID` | Account ID from step 1. Required: Pages config cannot set `account_id`. API pins it in `apps/api/wrangler.toml`. |
...
| `Failed to automatically retrieve account IDs` | Token cannot list accounts. Set repo (and `production` env) secret `CLOUDFLARE_ACCOUNT_ID`. Do not put `account_id` in the Pages `wrangler.toml`. |
